...
// Instance of a class
const user: User = new User("John", "john@email.com", 123);
const admin: Admin = new Admin("Mark", "mark@email.com", 123, 23);
user.lastName = "last";
...
type UserG = Identifiable & {
  name: string;
  email: string;
};

type BookG = Identifiable & {
  title: string;
  ISBN: number;
};

const usersRepository = new Repository<UserG>();
const booksRepository = new Repository<BookG>();
...
// new(...args: any[]) => {} -> constructor

type Constructor = new (...args: any[]) => {};

function TimeStamp<T extends Constructor>(BaseClass: T) {
...
const newUser: UserWithTimestamp = new UserWithTimestamp("Alice", 30);
console.log(newUser);
newUser.displayInfo();
